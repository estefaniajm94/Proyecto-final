...
class TrainingStartFragment : BaseFragment<FragmentTrainingStartBinding>(
    ...
    override fun onBoundView(savedInstanceState: Bundle?) {
        ...
        collectOnStarted(viewModel.uiState) { state ->
            ...
            binding.tvTotalQuestions.text =
                getString(R.string.training_total_questions, state.selectedLevelQuestionCount)
    ...
    private fun updateBubbleSelection(selectedLevel: TrainingLevel) {
        updateBubble(binding.cardLevelBeginner, binding.tvLevelBeginner, selectedLevel == TrainingLevel.BEGINNER)
        updateBubble(binding.cardLevelIntermediate, binding.tvLevelIntermediate, selectedLevel == TrainingLevel.INTERMEDIATE)
        updateBubble(binding.cardLevelAdvanced, binding.tvLevelAdvanced, selectedLevel == TrainingLevel.ADVANCED)
    }

    private fun updateBubble(card: MaterialCardView, titleView: TextView, isSelected: Boolean) {

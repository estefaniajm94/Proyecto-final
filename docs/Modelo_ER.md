# Modelo Entidad-Relación — AntiPhishingCoach

Base de datos local Room: `antiphishing_coach.db` · Versión de esquema: 1
...
erDiagram
    ...
    incidents {
        ...
        TEXT    sourceApp       "NOT NULL · enum: WHATSAPP, EMAIL, BROWSER, OTHER"
...
| `sourceApp`    | TEXT        | NOT NULL             | App de origen: `WHATSAPP`, `EMAIL`, `BROWSER`, `OTHER` |
...
- El borrado en cascada garantiza que `DELETE FROM incidents` elimina todos los datos
  asociados sin queries adicionales. Lo usa `ClearLocalDataUseCase`.
...
Room materializa estas relaciones mediante `@Transaction` para garantizar
consistencia cuando una lectura logica requiere multiples queries SQL internas.

---

## TypeConverter: `StringListConverter`

`recommendationCodes` en `analysis_results` almacena una `List<String>`
serializada como JSON mediante Gson:

```
["REC_VERIFY_SENDER", "REC_CHECK_LINKS", "REC_DO_NOT_CLICK"]
```

SQLite no tiene tipo array nativo. Se eligio JSON sobre columnas separadas
porque el numero de recomendaciones por analisis es variable (0-N)
y las recomendaciones se leen siempre juntas, nunca de forma individual.
...
| 3 tablas relacionadas | Tabla unica con columnas JSON | Las senales son consultables individualmente; una tabla plana lo impediria |
| `fallbackToDestructiveMigration` | Migraciones explicitas | Version 1 del esquema; sin datos de produccion que proteger |
